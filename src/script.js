...
console.log("выборка из объекта");
...
products.forEach((cost) => (cost.price *= 0.85));
...
console.log(
  product.filter(
    (photoProduct) =>
      "photos" in photoProduct && photoProduct.photos.length !== 0
  )
);
console.log("сортировка по цене");
console.log(
  product.sort((startSort, endSort) => startSort.price - endSort.price)
);
...
console.log("объединение массивов дней недели");
en.length === ru.length ? mixDiv() : console.log("массивы не равны");
